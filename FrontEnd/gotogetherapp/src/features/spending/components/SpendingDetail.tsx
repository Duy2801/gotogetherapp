...
const SpendingDetail = ({ navigation, route }: { navigation: any; route: any }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const tripRequests = ids.map((id: string) => tripDetailApi.getTripDetail(id));
      const expenseRequests = ids.map((id: string) => tripDetailApi.getTripExpenses(id, { page: 1, limit: 100 }));

      const tripResults = await Promise.all(tripRequests);
      const expenseResults = await Promise.all(expenseRequests);

      const tripList = tripResults.map(res => (res as any)?.data ?? res);
      setTripDataList(tripList);

      const allExpenses = expenseResults.flatMap(res => (res as any)?.data?.expenses ?? (res as any)?.expenses ?? []);
    ...
    expenses.forEach(expense => {
      expense.splits?.forEach(split => {
        if (split.userId !== expense.paidBy.id) {
          ...
          if (!from || !to) return;
          if (settledMap.has(key)) settledMap.get(key)!.amount += split.amount;
          else
            settledMap.set(key, {
              from,
              to,
  ...
  const memberBreakdown = useMemo(() => {
    ...
    tripDataList.forEach(trip => {
      trip.members?.forEach(member => {
        if (!breakdown[member.userId]) breakdown[member.userId] = { paid: 0, owes: 0, member };
      });
    });

    expenses.forEach(expense => {
      if (breakdown[expense.paidBy.id]) breakdown[expense.paidBy.id].paid += expense.amount;
      expense.splits?.forEach(split => {
        if (breakdown[split.userId]) breakdown[split.userId].owes += split.amount;
      });
    });
  ...
  return (
    <SafeAreaView style={styles.screen} edges={["top", "left", "right"]}>
      ...
      <ScrollView contentContainerStyle={styles.contentContainer} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={[PRIMARY_COLOR]} />}>
        {selectedTab === 'info' && <MultiTripInfoTab tripDataList={tripDataList} memberBreakdown={memberBreakdown} />}
          ...
          return (
            <View key={item.member.id} style={[styles.breakdownRow, idx > 0 && styles.breakdownRowBorder]}>
              <View style={styles.memberCol}>
                {item.member.avatar ? <Image source={{ uri: item.member.avatar }} style={styles.memberAvatar} /> : <View style={[styles.memberAvatar, styles.avatarFallback]}><Text style={styles.avatarText}>{item.member.fullName.charAt(0).toUpperCase()}</Text></View>}
              ...
              <Text style={[styles.balanceValue, { color: isPositive ? '#10B981' : '#EF4444' }]}>{isPositive ? '+' : ''}{formatMoney(balance)}</Text>
            </View>
          );
        })}
      </View>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.cardBody}>
          ...
          {memberBreakdown.map((item, idx) => {
            ...
            return (
              <View key={item.member.id} style={[styles.breakdownRow, idx > 0 && styles.breakdownRowBorder]}>
                <View style={styles.memberCol}>{item.member.avatar ? <Image source={{ uri: item.member.avatar }} style={styles.memberAvatar} /> : <View style={[styles.memberAvatar, styles.avatarFallback]}><Text style={styles.avatarText}>{item.member.fullName.charAt(0).toUpperCase()}</Text></View>}<Text style={styles.memberNameSmall}>{item.member.fullName}</Text></View>
                <Text style={styles.breakdownValue}>{formatMoney(item.paid)}</Text>
                <Text style={styles.breakdownValue}>{formatMoney(item.owes)}</Text>
                <Text style={[styles.balanceValue, { color: isPositive ? '#10B981' : '#EF4444' }]}>{isPositive ? '+' : ''}{formatMoney(balance)}</Text>
              </View>
...
const ExpensesTab = ({ expenses }: { expenses: Expense[] }) => {
  ...
  return (
    <View style={styles.tabContent}>
      {expenses.length === 0 ? (
        <View style={styles.emptyCard}><FontAwesome6 name="receipt" size={32} color="#D1D5DB" iconStyle="solid" /><Text style={styles.emptyText}>{t('expense.noExpenses')}</Text></View>
      ) : (
        expenses.map(expense => (
          <View key={expense.id} style={styles.expenseCard}>
            <View style={styles.expenseHeader}><View style={styles.expenseCategory}><FontAwesome6 name="tag" size={12} color={PRIMARY_COLOR} iconStyle="solid" /><Text style={styles.expenseCategoryText}>{expense.category?.name || t('common.other')}</Text></View><Text style={styles.expenseAmount}>{formatMoney(expense.amount)}</Text></View>
            <Text style={styles.expenseDesc}>{expense.description}</Text>
            <Text style={styles.expenseDate}>{formatDate(expense.date)}</Text>

            <View style={styles.payerRow}>
              {expense.paidBy.avatar ? <Image source={{ uri: expense.paidBy.avatar }} style={styles.payerAvatar} /> : <View style={[styles.payerAvatar, styles.avatarFallback]}><Text style={styles.avatarText}>{expense.paidBy.fullName.charAt(0).toUpperCase()}</Text></View>}
              <View style={styles.payerInfo}><Text style={styles.payerLabel}>{t('expense.paidBy')}</Text><Text style={styles.payerName}>{expense.paidBy.fullName}</Text></View>
            </View>

            {expense.splits && expense.splits.length > 0 && (
              <View style={styles.splitsContainer}>
                <Text style={styles.splitsTitle}>{t('expense.splitWith')} {expense.splits.length} {t('common.user')} ({formatMoney(expense.amount / expense.splits.length)}):</Text>
                {expense.splits.map(split => (
                  <View key={split.id} style={styles.splitRow}>
                    <View style={styles.splitUserInfo}>{split.user.avatar ? <Image source={{ uri: split.user.avatar }} style={styles.splitAvatar} /> : <View style={[styles.splitAvatar, styles.avatarFallback]}><Text style={styles.avatarSmall}>{split.user.fullName.charAt(0).toUpperCase()}</Text></View>}<Text style={styles.splitUser}>{split.user.fullName}</Text></View>
                    <View style={styles.splitStatusCol}><Text style={styles.splitAmount}>{formatMoney(split.amount)}</Text><View style={[styles.splitStatusBadge, { backgroundColor: split.confirmed ? '#ECFDF5' : split.isPaid ? '#FEF3C7' : '#FEE2E2' }]}><Text style={[styles.splitStatusText, { color: split.confirmed ? '#059669' : split.isPaid ? '#92400E' : '#DC2626' }]}>{split.confirmed ? t('common.success') : split.isPaid ? t('common.yes') : t('common.no')}</Text></View></View>
                  </View>
                ))}
              </View>
            )}
...
const SettlementTab = ({ items, totalSpent }: { items: SettlementItem[]; totalSpent: number }) => {
  ...
  return (
    <View style={styles.tabContent}>
      <View style={styles.card}>
        <View style={styles.cardHeader}><FontAwesome6 name="scale-balanced" size={16} color={PRIMARY_COLOR} iconStyle="solid" /><Text style={styles.cardTitle}>{t('spending.overviewTitle')}</Text></View>
        <View style={styles.cardBody}>
          <Text style={styles.settlementSummary}>{t('spending.spent')}: {formatMoney(totalSpent)}</Text>
          {items.length === 0 ? <Text style={styles.emptySettlement}>{t('spending.noPayments')}</Text> : (
            <>
              <Text style={styles.settlementInfo}>{items.length} {t('spending.paymentTitle')}</Text>
              {items.map((item, idx) => (
                <View key={idx} style={styles.settlementRow}>
                  <View style={styles.settlementPeople}>{item.from.avatar ? <Image source={{ uri: item.from.avatar }} style={styles.settlementAvatar} /> : <View style={[styles.settlementAvatar, styles.avatarFallback]}><Text style={styles.avatarSmall}>{item.from.fullName.charAt(0).toUpperCase()}</Text></View>}<Text style={styles.settlementFromName}>{item.from.fullName}</Text><FontAwesome6 name="arrow-right" size={14} color="#9CA3AF" iconStyle="solid" style={styles.arrowIcon} />{item.to.avatar ? <Image source={{ uri: item.to.avatar }} style={styles.settlementAvatar} /> : <View style={[styles.settlementAvatar, styles.avatarFallback]}><Text style={styles.avatarSmall}>{item.to.fullName.charAt(0).toUpperCase()}</Text></View>}<Text style={styles.settlementToName}>{item.to.fullName}</Text></View>
                  <View style={styles.settlementRight}><Text style={[styles.settlementAmount, { color: getStatusColor(item.status) }]}>{formatMoney(item.amount)}</Text><View style={styles.statusRow}><View style={[styles.statusDot, { backgroundColor: getStatusColor(item.status) }]} /><Text style={[styles.statusLabel, { color: getStatusColor(item.status) }]}>{item.status === 'CONFIRMED' ? t('common.success') : item.status === 'PAID' ? t('common.yes') : t('common.no')}</Text></View></View>
                </View>
              ))}
            </>
          )}
